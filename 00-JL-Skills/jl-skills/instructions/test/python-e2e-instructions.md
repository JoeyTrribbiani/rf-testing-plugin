...
> **角色适配**:
> - **测试工程师**: 生成可执行的 Robot Framework 或 Python 测试脚本
...
**即将生成的文件**:
- `docs/modules/{所属模块}/e2e_test.py` (此处的{所属模块}为您在上一环节锁定的业务模块名)
...
**生成的文件**: `docs/modules/{所属模块}/e2e_test.py`
...
- 回复 **确认** → 测试用例生成全流程结束！相关产物已全部保存在 `docs/modules/{模块名}/` 下。
- 回复 **调整脚本: [具体说明]** → 我将修改脚本
