...
```mermaid
flowchart TD
    A[API入口<br/>POST /api/orders] --> B[业务逻辑层<br/>参数校验]
    B --> C[领域规则层<br/>业务规则校验]
    B --> D[数据层<br/>订单落库]
    B --> E[外部服务<br/>支付调用]
...
| 层级 | 测试类型 | 验证点 | 测试方法 |
|------|---------|--------|---------|
| API入口 | 接口测试 | 请求响应格式、状态码 | Postman/cURL |
| 业务逻辑层 | 单元测试 | 参数校验逻辑 | JUnit |
| 领域规则层 | 集成测试 | 业务规则正确性 | 集成测试 |
| 数据层 | 数据校验 | 数据库状态一致性 | SQL验证 |
| 外部服务 | Mock测试 | 外部调用异常处理 | Mockito |

### 关键测试验证点

1. **输入验证**: 必填字段、数据格式、边界值
2. **业务规则**: 库存检查、金额计算、状态流转
3. **数据一致性**: 订单创建后数据库状态
4. **异常处理**: 外部服务超时/失败的容错处理
````
